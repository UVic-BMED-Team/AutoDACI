...
{
	HAL_GPIO_WritePin( motor->GPIO_Dir_Port, motor->GPIO_Dir_Pin, GPIO_PIN_SET );
	HAL_Delay(1);
...
void motor_init(Motor_HandleTypeDef* motor)
{
	...
	SD_Step_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
	...
	SD_Dir_InitStruct.Speed = GPIO_SPEED_FREQ_MEDIUM;
...
//			  {0xF0, 0xC4, 0x0C, 0x00, 0x1E}  // PWMCONF
			{0x94, 0x00, 0xFF, 0x00, 0x40}, // TCOOLTHRS -> TSTEP based threshold = 55 (Datasheet Page 38)
			{0x80, 0x00, 0x00, 0x00, 0x80},
			{0xED, 0x00, 0x80, 0x00, 0x01}, // CoolConf Stallguard SGT (need adapt to get a StallGuard2 event) (datasheet page 52)
			{SHORTCONF/*0x89*/, 0x00, 0x01, 0x06, 0x0A}, // SHORTCONF
			{IHOLD_IRUN/*0x90*/, 0x00, 0x04, 0x05, 0x01}, // IHOLD_IRUN
			//{0x91, 0x00, 0x00, 0x00, 0x0A}, // TPOWERDOWN
			{0x91, 0x00, 0x00, 0x00, 0x04}, // TPOWERDOWN
